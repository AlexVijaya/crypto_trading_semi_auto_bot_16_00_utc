...
def get_base_of_trading_pair(row):
    trading_pair_list_quote_and_base = row.split('/')
    base=trading_pair_list_quote_and_base[0]
    return (base)
...
def add_exchange_count2(df, exchange_map):
    ...
    return result_df
def add_base_column(df):
    bases_list = []
    base=""
    for index, row in df.iterrows():
        try:
            trading_pair_list_quote_and_base = row['base_slash_exchange'].split('/')
            base = trading_pair_list_quote_and_base[0].replace("/USDT", "")
            if " " in base:
                base=base.replace(" ","_")
        except:
            traceback.print_exc()
        bases_list.append(base)
    df['base_of_trading_pair'] = bases_list
    return df
...
def remove_trading_pairs_which_contain_stablecoin_as_base(filtered_pairs,stablecoin_bases_with_slash_list):
    ...
    return filtered_pairs
def get_base_of_trading_pair2(row):
    trading_pair_list_quote_and_base = row.split('/')
    base = trading_pair_list_quote_and_base[0].replace("/USDT", "")
    return base
...
if __name__=="__main__":
    ...
    df_with_strings_where_each_pair_is_traded.fillna('', inplace=True)
    df_with_strings_where_each_pair_is_traded['base_slash_exchange'] = df_with_strings_where_each_pair_is_traded.index
    print("df_with_strings_where_each_pair_is_traded.columns")
    print(df_with_strings_where_each_pair_is_traded.columns)

    # df_with_strings_where_each_pair_is_traded['base_of_trading_pair'] = \
    #     df_with_strings_where_each_pair_is_traded['base_slash_exchange'].apply(
    #         get_base_of_trading_pair)

    # df_with_strings_where_each_pair_is_traded['base_of_trading_pair'] =\
    #     df_with_strings_where_each_pair_is_traded['base_slash_exchange'].apply(get_base_of_trading_pair)

    df_with_strings_where_each_pair_is_traded=add_base_column(df_with_strings_where_each_pair_is_traded)
